...
const Discover = () => {
  ...
  const mockPosts = [
    ...
    },
    {
      ...
      time: '8h',
      content: 'New yield farming opportunities just dropped! 🌾 APY rates are looking incredibly attractive this week. Remember: DYOR and never invest more than you can afford to lose.\n\n#DeFi #YieldFarming #CryptoTips',
      likes: 567,
      replies: 78,
      retweets: 145,
      trending: false
    },
    {
      ...
      time: '12h',
      content: 'Just shipped a new smart contract on Polygon! ⚡ Gas fees are so low it feels like magic. The future of dApps is multi-chain.\n\n#Polygon #SmartContracts #Web3Dev',
    ...
    },
    {
      ...
      time: '1d',
      content: 'Excited to announce our new partnership with leading DeFi protocols! 🎉 This collaboration will bring even more utility to our platform. Stay tuned for major updates! 🔜\n\n#StellaFi #Partnership #DeFi',
      likes: 1123,
      replies: 234,
      retweets: 567,
      trending: true
              ...
              <div className="flex space-x-3 overflow-x-auto">
                {[
                  { 
                    ...
                    { 
                      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.828 14.828a4 4 0 01-5.656 0M9 10h1m4 0h1m-6 4h8m-5-5V5a3 3 0 116 0v5H9z" /></svg>, 
                      label: 'Emoji',
                  ...
                  {tweetText.length > 0 && (
                    <div className="flex items-center space-x-2">
                      <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full border-2 relative ${
                        tweetText.length > 260 ? 'border-red-500' : 
                        tweetText.length > 240 ? 'border-yellow-500' : 'border-twitter-blue'
                      }`}>
                        <div 
                          className={`absolute inset-0.5 rounded-full transition-all duration-200 ${
                            tweetText.length > 260 ? 'bg-red-500' : 
                            tweetText.length > 240 ? 'bg-yellow-500' : 'bg-twitter-blue'
                          }`}
                          style={{
                            transform: `rotate(${(tweetText.length / 280) * 360}deg)`,
